...
public class CreateTicketBean implements Serializable {
    ...
    public void createTicket() {
        ...
        if (selectedAreas.size() != 0 && selectedProduct.name().length() != 0) {
            ...
            message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Ticket Added",
                    selectedProduct.name() + "\n" + areaMessage);
        } else if (selectedAreas.size() == 0 || selectedProduct.name().length() == 0 || zendeskId == 0) {
            message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Validation error",
                    "Choose product or Area or Zendesk ID");
        }
    ...
    public ArrayList<TicketEntity> getCreatedTickets() {
        if (loginBean.getUserRole().equals("admin")) {
            ...
            return ticketService.getUserCreatedTicketsByDate(loginBean.getUser_id(), calendarBean.getDate());
        } else {
...
//	TODO need change logic
    public void deleteClosedTickets() {
		...
		if (loginBean.getUserRole().equals("admin")) {
			solvedList = ticketService.getUserClosedTicketsByDate(calendarBean.getDate());
		} else if (loginBean.getUserRole().equals("user")) {
